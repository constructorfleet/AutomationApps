...
class HassmqttPlugin:
    ...
    def __init__(self, ad, name, logger, error, loglevel, args):
        ...
        self.mqtt_client = mqtt.Client(client_id=mqtt_client_id, clean_session=mqtt_session,
    ...
    def stop(self):
        ...
        if self.initialized:
            self.AD.log("INFO",
                        "{}: Stoping MQTT Plugin and Unsubcribing from URL {}:{}".format(self.name,
    ...
    def mqtt_on_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:  # means connection was successful
            self.mqtt_client.publish(self.mqtt_on_connect_topic, self.mqtt_on_connect_payload,
                                     self.mqtt_qos)

            self.AD.log("INFO", "{}: Connected to Broker at URL {}:{}".format(self.name,
                                                                              self.mqtt_client_host,
                                                                              self.mqtt_client_port))
            for topic in self.mqtt_client_topics:
                ...
                if result[0] == 0:
                    ...
                    if topic == self.mqtt_metadata['plugin_topic']:
                        self.AD.log("CRITICAL",
                                    "{}: Subscription to Plugin Internal Topic Unsucessful. Please check Broker and Restart AD".format(
                                        self.name))
                    else:
                        self.log(
                            "{}: Subscription to Topic {} Unsucessful, as Client not currently connected".format(
                                self.name, topic))
        ...
        if err_msg != "":  # means there was an error
            self.AD.log("CRITICAL",
                        "{}: Could not complete MQTT Plugin initialization, for {}".format(
                            self.name, err_msg))
    ...
    def mqtt_on_disconnect(self, client, userdata, rc):
        if rc != 0 and not self.stopping:  # unexpected disconnection
            self.initialized = False
            self.AD.log("CRITICAL",
                        "{}: MQTT Client Disconnected Abruptly. Will attempt reconnection".format(
                            self.name))
        return
    ...
    def mqtt_service(self, service, **kwargs):
        if not self.initialized:
            self.AD.log("WARNING",
                        "{}: Attempt to call Mqtt Service while disconnected: {!r}".format(
                            self.name, service))
            return None
        ...
        if service == 'publish':
            ...
            if result[0] == 0:
                self.log(
                    "{}: Publishing Payload {} to Topic {} Successful".format(self.name, payload,
                                                                              topic))
        ...
        while not self.stopping:
            while not self.initialized or not already_initialized:  # continue until initialization is successful
                if not already_initialized and not already_notified:  # if it had connected before, it need not run this. Run if just trying for the first time
                    try:
                        await asyncio.wait_for(utils.run_in_executor(self.AD.loop, self.AD.executor,
                        ...
                        self.AD.log(
                            "CRITICAL",
                            "{}: Could not Complete Connection to Broker, please Ensure Broker at URL {}:{} is correct or broker not down and restart Appdaemon".format(
                                self.name, self.mqtt_client_host, self.mqtt_client_port))
                        self.mqtt_client.loop_stop()
                ...
                if self.initialized:  # meaning the plugin started as expected
                    ...
                    if not already_notified and already_initialized:
                        ...
                        self.AD.log("CRITICAL",
                                    "{}: MQTT Plugin Stopped Unexpectedly".format(self.name))
                        already_notified = True
                        already_initialized = False
                        first_time = False
                    if not already_initialized and not already_notified:
                        self.AD.log("CRITICAL",
                                    "{}: Could not complete MQTT Plugin initialization, trying again in 5 seconds".format(
                                        self.name))
                    else:
                        self.AD.log("CRITICAL",
                                    "{}: Unable to reinitialize MQTT Plugin, will keep trying again until complete".format(
                                        self.name))
        ...
        try:
            ...
            if first_time:
                ...
                if self.mqtt_client_tls_ca_cert != None:
                    self.mqtt_client.tls_set(self.mqtt_client_tls_ca_cert,
                ...
                self.mqtt_client.will_set(self.mqtt_will_topic, self.mqtt_will_payload,
                                          self.mqtt_qos)
            ...
            self.AD.log("CRITICAL",
                        "{}: There was an error while trying to setup the Mqtt Service. Error was: {}".format(
                            self.name, e))
            self.AD.log("DEBUG",
                        "{}: There was an error while trying to setup the MQTT Service. Error: {}, with Traceback: {}".format(
                            self.name, e, traceback.format_exc()))
            self.log(
                '{}: There was an error while trying to setup the MQTT Service, with Traceback: {}'.format(
                    self.name, traceback.format_exc()), level='CRITICAL')
        except:
            self.AD.log("CRITICAL",
                        "{}: There was an error while trying to setup the Mqtt Service".format(
                            self.name))
            self.log(
                '{}: There was an error while trying to setup the MQTT Service, with Traceback: {}'.format(
                    self.name, traceback.format_exc()), level='CRITICAL')
