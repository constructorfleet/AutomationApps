...
class HassMqtt(appapi.AppDaemon):
    entities = Entities()

    def __init__(self, ad, name, logger, error, args, config, app_config, global_vars, ):

        super(HassMqtt, self).__init__(ad, name, logger, error, args, config, app_config, global_vars)
    ...
    def listen_state(self, cb, entity=None, **kwargs):
        ...
        if "namespace" in kwargs:
            del kwargs["namespace"]
        return super(HassMqtt, self).listen_state(namespace, cb, entity, **kwargs)

    def listen_event(self, cb, event=None, **kwargs):
        namespace = self._get_namespace(**kwargs)
        if "namespace" in kwargs:
            del kwargs["namespace"]
        return super(HassMqtt, self).listen_event(namespace, cb, event, **kwargs)
    ...
    def get_state(self, entity=None, **kwargs):
        ...
        if "namespace" in kwargs:
            del kwargs["namespace"]
        return super(HassMqtt, self).get_state(namespace, entity, **kwargs)
            ...
            try:
                ...
                if config['type'] == 'hassmqtt':
                    ...
                    self.AD.log('CRITICAL',
                                'Wrong Namespace {!r} selected for MQTT Service. Please use proper namespace before trying again'.format(
                                    namespace))
                    self.error(
                        'Could not execute Service Call, as wrong Namespace {!r} used'.format(
                            namespace))
                    return 'ERR'
        else:
