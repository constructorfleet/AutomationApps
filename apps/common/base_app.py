...
class BaseApp(hassmqtt.HassMqtt):
    ...
    def save_data(self, kwargs):
        self.warn("Saving %s" % str(self.data))
        with self._data_lock:
    ...
    def publish_service_call(self, domain, service, kwargs):
        self.warn("Publish Domain %s Service %s with args %s" % (domain, service, str(kwargs)))
    ...
    def publish_event(self, event, event_data, qos=0, retain=False, namespace='default'):
        self.warn("Publish Event %s Data %s " % (event, str(event_data)))
